...
// Kernel szamolas
int kernel_comp(dtype kernel[3][3],
                const int filter1[3][3], const int filter2[3][3]) {
    int gx = 0, gy = 0;
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
...
void compute(hls::stream<dtype>& in_stream,
             hls::stream<dtype>& out_stream,
             int filter1[3][3], int filter2[3][3]) {
...
void sliding_window(dtype* img, dtype* result, int filter1[3][3],
                    int filter2[3][3]) {
#pragma HLS DATAFLOW 
